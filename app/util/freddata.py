"""
freddata.py - Interface into the St. Louis Federal Reserve Bank's FRED API.
...
class Fred(object):
    ...
    def __init__(self):
        ...
        """
        self.logger = Logger.get_logger("fred")

        try:
            self.api_key = os.environ["FRED_API_KEY"]
        except ValueError as e:
            self.logger.error("Unable to retrieve FRED_API_KEY: %s", e)
            self.api_key = None
...
class FredUtil(object):
    ...
    def average_fixed_mortgage(self, year: int, month: int=0, duration: int=30)-> Decimal:
        ...
        Args:
            ...
            month (int): Month being queried. If omitted, the average is for the entire year. [1-12]
            duration (int): Term of mortgage loan in years. [5, 15, 30]

        Returns:
            (Decimal): Average mortgage interest rate for the period, or None if not found.
        """
        if duration not in [5, 15, 30]:
            raise ValueError("duration must be one of 5, 15, or 30")

        if month < 0 or month > 12:
            raise ValueError("month must be in the range 1-12 (inclusive) or omitted")
        ...
        if month == 0:
            ...
            # Figure out last day of the month, accounting for leap-years (during my lifetime)
        ...
        # Retrieve data.
        result = self.fred.series_observations(f"MORTGAGE{duration}US", **params)
